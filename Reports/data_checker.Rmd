...
delivery_agents <- delivery_agents %>% 
  select(id, delivery_library_names, unit_3_teams) %>% 
  pivot_longer(2:3) %>% 
  mutate(value = case_when(
    is.na(value) & name == "delivery_library_names" ~ "Other AC staff or external",
    !str_detect(value, "Research|Special") & name == "unit_3_teams" ~ NA_character_,
    TRUE ~ value
  )) %>% 
  select(id, delivery_library_names = value) %>% 
...
The following teams have not entered data through the form within the last 60 days: **`r dplyr::setdiff(expected_teams$delivery_team, data_checker$delivery_agent_or_location)`**
