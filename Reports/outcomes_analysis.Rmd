...
params:
  year: 2021
  month: July
  outcome: "Kia ora Tāmaki Makaurau"
  outcome_owner: " for Judith Waaka"
...
common_table_headings <- c('Sessions', 'Participation (18+)', 'Participation (under 18)', 'Total Duration')
...
this_outcome_waffle %>% 
  ...
  waffle(
    colors = c("blue", "grey"),
    legend_pos = "bottom",
    title = paste0(with_this_outcome, "% of the in-scope libraries activities\ndelivered in ", paste0(params$month, " ", params$year), " were\nsaid to have delivered against\nthe ", params$outcome, " outcome"),
...
outcome_explorer_data <- outcome_data %>% 
  ...
  mutate(in_which_local_board_was_the_session_delivered = case_when(
    is.na(in_which_local_board_was_the_session_delivered) ~ "Online",
    !is.na(in_which_local_board_was_the_session_delivered) ~ as.character(in_which_local_board_was_the_session_delivered)
  )) %>% 
  mutate(location = paste0(in_which_local_board_was_the_session_delivered, ": ", location), delivery_datetime = ymd_hms(delivery_datetime), delivery_date = stamp("30 November at 4.30pm", orders="dBIMp", quiet = TRUE)(delivery_datetime), participants = (rowSums(across(starts_with("how_many_")), na.rm = TRUE))) %>% 
...
Looking specifically at the activities in `r params$month` `r params$year` that were said to have delivered against the `r params$outcome` outcome, there were **`r prettyNum(this_outcome$participants_adult, big.mark = ",")` adult participants** and **`r prettyNum(this_outcome$participants_children, big.mark = ",")` child participants** across these sessions. It's important to note that the number of participants does not necessarily reflect the number of people who attended theses sessions, as the same person can attend multiple sessions in the same month and is therefore counted as a participant each time.

These sessions ran for a total of **`r prettyNum(round(this_outcome$duration), big.mark = ",")` hours**.
...
The following table shows, for each of these categories, the number sessions said to deliver against the `r params$outcome` outcome in `r paste0(params$month, " ", params$year)`, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly.
...
`r if(show_outcome_stats_table) {"This outcome area has several outcomes associated with it. The table below shows how many sessions were said to have delivered against each outcome, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly."}`
...
`r if(params$outcome == "Literacy") {"### Adult Literacy"}`

`r if(params$outcome == "Literacy") {"At the broadest level, literacy programmes are designed for particular age groups: adults, youths, and children. The table below shows how many sessions designed to *exclusively* benefit adults were said to have delivered against each outcome, the number of adults participants (aged 18+) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly."}`


```{r adult_literacy_outcomes_data}
if(params$outcome == "Literacy") {
  literacy_data <- outcome_data %>%
    select(id, starts_with(c("how_many_", "what_was_the_duration_")), outcome_attribute) %>% 
    left_join(concat_rows(outcome_data, age_group), by = "id")
  
  adult_literacy_data <- literacy_data %>% 
    filter(!str_detect(str_to_lower(age_group), "youth") & !str_detect(str_to_lower(age_group), "children")) %>% 
    distinct(id, outcome_attribute, .keep_all = TRUE) %>% 
    group_by(outcome_attribute) %>%
    summarise(
      sessions = n_distinct(id),
      participants_adult = sum(how_many_adults_aged_18_attended_in_person_at_this_session, na.rm = TRUE),
...
```{r adult_literacy_outcomes_table}
if(params$outcome == "Literacy") {
  caption <- paste0('Summary information per ', params$outcome, ' outcome for the adult ', params$outcome, ' activities delivered in ', params$month, ' ', params$year)
  
  DT::datatable(
    adult_literacy_data,
    colnames = c('Outcome', 'Sessions', 'Participation (18+)', 'Total Duration'),
    caption = caption,
    options = list(dom = 't'),
    rownames = FALSE
  )
...
`r if(params$outcome == "Literacy") {"And here is a graphical representation of the same information:"}`

```{r adult_literacy_outcomes_chart}
if(params$outcome == "Literacy") {
  adult_literacy_data %>% 
    ...
    mutate(metric = factor(metric, levels = c("Sessions", "Total Duration (hours)", "Participants (18+)"))) %>%
    ggplot(mapping = aes(x = tidytext::reorder_within(outcome_attribute, value, metric), y = value)) +
    geom_col(fill = "blue") +
...
`r if(params$outcome == "Literacy") {"### Youth Literacy"}`

`r if(params$outcome == "Literacy") {"The table below shows how many sessions designed to *exclusively* benefit youths were said to have delivered against each outcome, the number of adults participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly."}`

```{r youth_literacy_outcomes_data}
if(params$outcome == "Literacy") {
  youths_literacy_data <- literacy_data %>% 
    filter(!str_detect(str_to_lower(age_group), "adult") & !str_detect(str_to_lower(age_group), "children")) %>% 
    distinct(id, outcome_attribute, .keep_all = TRUE) %>% 
    summarise_data(outcome_attribute)
}
```

```{r youth_literacy_outcomes_table}
if(params$outcome == "Literacy") {
  caption <- paste0('Summary information per ', params$outcome, ' outcome for the youth ', params$outcome, ' activities delivered in ', params$month, ' ', params$year)
  
youths_literacy_data %>% 
  basic_table("Outcome", caption)
}
```

`r if(params$outcome == "Literacy") {"And here is a graphical representation of the same information:"}`

```{r youth_literacy_outcomes_chart}
if(params$outcome == "Literacy") {
  youths_literacy_data %>% 
    basic_chart(outcome_attribute)
}
...
`r if(params$outcome == "Literacy") {"### Children\'s Literacy"}`

`r if(params$outcome == "Literacy") {"The table below shows how many sessions designed to benefit children (pre-school and primary) were said to have delivered against each outcome, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly."}`

```{r child_literacy_outcomes_data}
if(params$outcome == "Literacy") {
  child_literacy_data <- literacy_data %>% 
...
```{r child_literacy_outcomes_table}
if(params$outcome == "Literacy") {
  caption <- paste0('Summary information per ', params$outcome, ' outcome for the children\'s ', params$outcome, ' activities delivered in ', params$month, ' ', params$year)
...
```{r child_literacy_outcomes_chart}
if(params$outcome == "Literacy") {
  child_literacy_data %>% 
    basic_chart(outcome_attribute)
}
...
The table below shows, for each of these Local Boards, the number sessions said to deliver against the `r params$outcome` outcome, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly.
...
The table below shows, for each of these types of delivery agents, the number sessions said to deliver against the `r params$outcome` outcome, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly.
...
The following table shows, for each placed-based team, the number sessions said to deliver against the `r params$outcome` outcome, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly.
...
The following table shows, for each regional Unit, the number sessions said to deliver against the `r params$outcome` outcome, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly.
...
The following table shows, for each age group, the number sessions said to deliver against the `r params$outcome` outcome, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly.
...
The following table shows, for each group of interest, the number sessions said to deliver against the `r params$outcome` outcome, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly.
...
The following table shows, for each language other than English, the number sessions said to deliver against the `r params$outcome` outcome, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly.
...
The following table shows, for each regional theme, the number sessions said to deliver against the `r params$outcome` outcome, the number of adult participants (aged 18+) and child participants (aged under 18) across these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly.
