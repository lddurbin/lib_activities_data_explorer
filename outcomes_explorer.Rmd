...
```{r overall_waffle}
overall_waffle <- tibble(
  group = c("With", "Without"),
  value = c(outcome_submissions, (monthly_submissions-outcome_submissions))
  ) %>% 
  arrange(desc(group)) %>%
  mutate(prop = round(value / sum(value) *100))

with_outcomes <- overall_waffle %>% 
  filter(group == "With") %>% 
  pull(prop)
  
overall_waffle %>% 
  ggplot(aes(fill = group, values = prop)) +
  waffle::geom_waffle() +
  scale_fill_manual(
    name = NULL,
    values = c("blue", "grey"),
    labels = c("Outcomes", "No outcomes")
  ) +
  coord_equal() +
  theme_void() +
  ggtitle(paste0(with_outcomes, "% of the in-scope libraries activities delivered in ", paste0(params$month, " ", params$year), "\nwere said to have delivered against at least one of the outcomes"))
```
...
this_outcome_waffle %>% 
  ...
  scale_fill_manual(
    name = NULL,
    values = c("blue", "grey"),
    labels = c(params$outcome, paste0("Not ", params$outcome))
  ) +
  coord_equal() +
  theme_void() +
  ggtitle(paste0(with_this_outcome, "% of the in-scope libraries activities delivered in ", paste0(params$month, " ", params$year), "\nwere said to have delivered against the ", params$outcome, " outcome"))
```


Looking specifically at the activities in `r params$month` `r params$year` that were said to have delivered against the `r params$outcome` outcome, **`r prettyNum(this_outcome$participants_adult, big.mark = ",")` adults** and **`r prettyNum(this_outcome$participants_children, big.mark = ",")` children** attended these sessions. These sessions ran for a total of **`r prettyNum(this_outcome$duration, big.mark = ",")` hours**.
...
<!-- And here is a graphical representation of the same information, split by region: -->

```{r locations_chart}

# locations_charts <- outcome_data %>% 
#   distinct(id, outcome, .keep_all = TRUE) %>% 
#   group_by(in_which_local_board_was_the_session_delivered, location) %>%
#   summarise(
#     sessions = n_distinct(id),
#     participants_adult = sum(how_many_adults_aged_18_attended_in_person_at_this_session, na.rm = TRUE),
#     participants_children = sum(how_many_children_aged_under_18_attended_in_person_at_this_session, na.rm = TRUE),
...
#     .groups = "drop"
#   ) %>% 
#   left_join(read_csv("data/local_board_groups.csv", col_types = "cdcc"), by = c("in_which_local_board_was_the_session_delivered" = "local_board"))
# 
# locations_charts %>% 
#   filter(local_board_region == "CE") %>% 
#   select(location, sessions, participants_adult, participants_children, duration) %>% 
#   basic_chart(location) +
#   ggtitle("Central-East")
# 
# locations_charts %>% 
#   filter(local_board_region == "NW") %>% 
#   select(location, sessions, participants_adult, participants_children, duration) %>% 
#   basic_chart(location) +
#   ggtitle("North-West")
# 
# locations_charts %>% 
#   filter(local_board_region == "S") %>% 
#   select(location, sessions, participants_adult, participants_children, duration) %>% 
#   basic_chart(location) +
#   ggtitle("South")
```
...
* [`r other_outcomes[1]`](`r paste0(other_reports[1], ".html")`)
* [`r other_outcomes[2]`](`r paste0(other_reports[2], ".html")`)
* [`r other_outcomes[3]`](`r paste0(other_reports[3], ".html")`)
* [`r other_outcomes[4]`](`r paste0(other_reports[4], ".html")`)
* [`r other_outcomes[5]`](`r paste0(other_reports[5], ".html")`)
