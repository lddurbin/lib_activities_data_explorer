...
In `r params$month` `r params$year`, **`r prettyNum(monthly_submissions, big.mark = ",")`** in-scope programmes, events, and Book a Librarian sessions were delivered across Tāmaki Makaurau. You can browse through the complete dataset by downloading the [Data Explorer]().
...
overall_pie <- tibble(
  ...
  value = c(outcome_submissions, (monthly_submissions-outcome_submissions))
)

overall_pie <- overall_pie %>% 
  arrange(desc(group)) %>%
  mutate(prop = value / sum(overall_pie$value) *100) %>%
  mutate(ypos = cumsum(prop)- 0.5*prop )

ggplot(overall_pie, aes(x="", y=prop, fill=group)) +
  geom_bar(stat="identity", width=1, color="white") +
  coord_polar("y", start=0) +
  theme_void() + 
  theme(legend.position="none") +
  geom_text(aes(y = ypos, label = paste0(group, ":\n ", prettyNum(value, big.mark = ","))), color = "white", size=4) +
  scale_fill_brewer(palette="Set1") +
  ggtitle("How many sessions were delivered with outcomes, and how many\nwere delivered without outcomes?")
```

Of the `r prettyNum(monthly_submissions, big.mark = ",")` activities that were delivered in `r params$month` `r params$year`, **`r this_outcome %>% pull(sessions) %>% prettyNum(big.mark = ",")`** were said to have delivered against the [`r params$outcome` outcome](`r params$outcome_url`). In other words, **`r round((this_outcome$sessions/monthly_submissions)*100)`%** of the in-scope libraries activities delivered in `r params$month` `r params$year` were reported as having delivered against the `r params$outcome` outcome.

```{r outcome_pie}
outcome_pie <- tibble(
  group = c(params$outcome, paste0("Not ", params$outcome)),
  value = c(this_outcome$sessions, (monthly_submissions-this_outcome$sessions))
)

outcome_pie <- outcome_pie %>% 
  arrange(desc(group)) %>%
  mutate(prop = value / sum(outcome_pie$value) *100) %>%
  mutate(ypos = cumsum(prop)- 0.5*prop )

ggplot(outcome_pie, aes(x="", y=prop, fill=group)) +
  geom_bar(stat="identity", width=1, color="white") +
  coord_polar("y", start=0) +
  theme_void() + 
  theme(legend.position="none") +
  geom_text(aes(y = ypos, label = paste0(group, ":\n ", prettyNum(value, big.mark = ","))), color = "white", size=4) +
  scale_fill_brewer(palette="Set1") +
  ggtitle("How many sessions were delivered against this outcome,\nand how many were not?")
```
...
In addition to English, we deliver programmes and events in a variety of other languages. In `r paste0(params$month, " ", params$year)`, a substantial part of **`r outcome_data %>% filter(!is.na(realm_language)) %>% distinct(id) %>% nrow() %>% prettyNum(big.mark = ",")`** `r params$outcome` sessions were delivered in languages other than English.
...
other_reports <- setdiff(str_split(Sys.getenv("OUTCOMES"), " ") %>% unlist(), params$outcome_filename) %>% sort()
```


To view similar information on sessions held in `r paste0(params$month, " ", params$year)` that were said to deliver against the other outcome areas, you can navigate to the relevant reports using the links below:

* [`r other_outcomes[1]`](`r paste0(here::here("Reports/"), other_reports[1], ".html")`)
* [`r other_outcomes[2]`](`r paste0(here::here("Reports/"), other_reports[2], ".html")`)
* [`r other_outcomes[3]`](`r paste0(here::here("Reports/"), other_reports[3], ".html")`)
* [`r other_outcomes[4]`](`r paste0(here::here("Reports/"), other_reports[4], ".html")`)
* [`r other_outcomes[5]`](`r paste0(here::here("Reports/"), other_reports[5], ".html")`)
