...
`r if(params$outcome == "Literacy") {"The table below shows how many sessions designed to *exclusively* benefit children (pre-school and primary) were said to have delivered against each outcome, the number of adults (aged 18+) and children (aged under 18) who participated in these sessions, and the total duration of these sessions in hours. You can click on the column headers to re-sort the table accordingly."}`

```{r child_literacy_outcomes_data}
if(params$outcome == "Literacy") {
  child_literacy_data <- literacy_data %>% 
    filter(!str_detect(str_to_lower(age_group), "adult") & !str_detect(str_to_lower(age_group), "youth")) %>% 
    distinct(id, outcome_attribute, .keep_all = TRUE) %>% 
    summarise_data(outcome_attribute)
}
```
