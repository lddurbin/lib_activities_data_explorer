...
data_explorer <- plyr::join_all(list(base_table, delivery_agents, age_groups, target_groups, realm_languages), by="id", type="left") %>% 
  ...
  mutate(
    ...
    offline_participants = how_many_adults_aged_18_attended_in_person_at_this_session + how_many_children_aged_under_18_attended_in_person_at_this_session
    ) %>% 
  select(-c(
    "NA",
    submission_time,
      ...
      offline_participants = colDef(
        ...
        style =
          JS(
            "function(rowInfo, colInfo, state) {
          var prevRow = state.pageRows[rowInfo.viewIndex - 1]
          if (prevRow && rowInfo.row['offline_participants'] === prevRow['offline_participants']) {
            return { visibility: 'hidden' }
          }
      }"
          )
