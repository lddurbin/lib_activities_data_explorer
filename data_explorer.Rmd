...
Data will first appear here on **9 July**. The data will be refreshed weekly on Fridays, as well as on the first working day of each month.
...
concat_libs <-
  delivery_agents %>% concat_rows(delivery_library_names)
...
```{r data_explorer_table, echo=FALSE, warning=FALSE, include=FALSE}
...
bscols(
  ...
  reactable(
    ...
    columns = list(
      ...
      "Talanoa" = colDef(
        header = with_tooltip("Tanalona", "Talanoa Pasifika framework"),
        style = function(value) {
          color <-
            if (value == "\u2718") {
              "#e00000"
